function startApp() {
  ...
  async function loadTemplates() {
    const [adTemplate, adsTemplate, credentialsTemplate] = await Promise.all([
      $.get('templates/ad.html'),
      $.get('templates/ads.html'),
      $.get('templates/credentials.html')
    ]);

    Handlebars.registerPartial('ad', adTemplate);
    templates['ads'] = Handlebars.compile(adsTemplate);
    templates['credentials'] = Handlebars.compile(credentialsTemplate);
  }
  ...
  function getAllAds() {
    ...
    function displayAllAdsSuccess(data) {
      ...
      let html = templates['ads'](context);
      container.html(html);

      let linkEdit = $(container).find('table').find('tr').find('td').find('a.edit');
      let linkDelete = $(container).find('table').find('tr').find('td').find('a.delete');

      linkEdit.click(getEditInfo);
      linkDelete.click(deleteAd);
    }
  }
  ...
  function getEditInfo() {
    let adId = $(this).parent().parent().attr('data-id');
