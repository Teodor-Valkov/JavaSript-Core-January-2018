let chirpsController = (() => {
  function allChirps(context) {
    ...
    let chirpsRequest = requester.get('appdata', `chirps?query={"author":"${username}"}`, 'kinvey');

    // array with many elements - user objects which have this username in their subscription property
    let followersRequest = requester.get('user', `?query={"subscriptions":"${username}"}`, 'kinvey');
    ...
    Promise.all([chirpsRequest, followersRequest])
      .then(([chirps, followers]) => {
        ...
        requester.get('appdata', `chirps?query={"author":{"$in": [${following}]}}&sort={"_kmd.ect": -1}`, 'kinvey')
          .then((feed) => {
  ...
  function myProfile(context) {
    ...
    let followingRequest = requester.get('user', `?query={"username":"${username}"}`, 'kinvey');

    // array with many elements - user objects which have this username in their subscription property
    let followersRequest = requester.get('user', `?query={"subscriptions":"${username}"}`, 'kinvey');

    // array with many elements - chirp objects which author is equal to this username
    let feedRequest = requester.get('appdata', `chirps?query={"author":"${username}"}&sort={"_kmd.ect": -1}`, 'kinvey');
  ...
  function otherProfile(context) {
    ...
    // array with many elements - chirp objects which author is equal to this username
    let chirpsRequest = requester.get('appdata', `chirps?query={"author":"${otherUsername}"}`, 'kinvey');

    // array with one element - user object which username is equal to this username    
    let followingRequest = requester.get('user', `?query={"username":"${otherUsername}"}`, 'kinvey')

    // array with many elements - user objects which have this username in their subscription property
    let followersRequest = requester.get('user', `?query={"subscriptions":"${otherUsername}"}`, 'kinvey');

    // array with many elements - chirp objects which author is equal to this username
    let feedRequest = requester.get('appdata', `chirps?query={"author":"${otherUsername}"}&sort={"_kmd.ect": -1}`, 'kinvey');
  ...
  function createChirpPost(context) {
    ...
    requester.post('appdata', 'chirps', 'kinvey', chirp)
      .then(function () {
