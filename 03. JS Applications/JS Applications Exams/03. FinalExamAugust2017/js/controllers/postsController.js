let postsController = (() => {
  ...
  function myPosts(context) {
    ...
    requester.get('appdata', `posts?query={"author":"${username}"}&sort={"_kmd.ect": -1}`, 'kinvey')
      .then((posts) => {
        let rank = 1;
  ...
  function postDetailsByPromiseAll(context) {
    ...
    let postRequest = requester.get('appdata', `posts/${postId}`, 'kinvey');
    let commentsRequest = requester.get('appdata', `comments?query={"postId":"${postId}"}&sort={"_kmd.ect": -1}`, 'kinvey');
